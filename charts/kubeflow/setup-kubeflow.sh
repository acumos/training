...
function get_dist() {
  if [[ $(bash --version | grep -c redhat-linux) -gt 0 ]]; then
    ...
    dist=windows
    echo "Sorry, Windows is not supported due to known issues with ksonnet."
    echo "It's recommended that you use this toolset from a VM (e.g. VirtualBox)"
    echo "and afer kubeflow install, interact with the kubeflow system via the VM."
...
function setup_prereqs() {
  trap 'fail' ERR
  if [[ "$k8sdist" == "openshift" && "$(which oc)" == "" ]]; then
#    curl -LO https://raw.githubusercontent.com/acumos/system-integration/master/tools/setup_openshift_client.sh
    bash setup_openshift_client.sh $master $user kubeflow
    export PATH=$(pwd):$PATH
    oc login -u admin -p any
  elif [[ "$(which kubectl)" == "" ]]; then
#    curl -LO https://raw.githubusercontent.com/acumos/kubernetes-client/master/deploy/private/setup_kubectl.sh
    bash setup_kubectl.sh $master $user kubeflow
  fi
  ...
  if [[ ! $(which ks) ]]; then
    ...
    if [[ "$dist" == "ubuntu" || "$dist" == "centos" ]]; then
      ...
      gzip -d ks_*.gz
      tar -xvf ks_*.tar
      sudo cp ks_*/ks /usr/bin/.
    elif [[ "$dist" == "macos" ]]; then
      curl -LO https://github.com/ksonnet/ksonnet/releases/download/v${KS_VERSION}/ks_${KS_VERSION}_darwin_amd64.tar.gz
      gzip -d ks_*.gz
      tar -xvf ks_*.tar
      sudo cp ks_*/ks /usr/local/bin/.
    elif [[ "$dist" == "windows" ]]; then
      curl -LO https://github.com/ksonnet/ksonnet/releases/download/v${KS_VERSION}/ks_${KS_VERSION}_windows_amd64.zip
      unzip -o ks_*.zip
      cp -f ks_*/ks .
      export PATH=$(pwd):$PATH
    else
...
clean() {
  ...
  get_dist
  if [[ $(bash --version | grep -c pc-msys) -gt 0 ]]; then
    export PATH=$(pwd):$PATH
    pid=$(ps -ef | awk '/kubectl/{print $2}' | head -1)
  else
    pid=$(ps -ef | awk '/port-forward/{print $2}' | head -1)
  fi
